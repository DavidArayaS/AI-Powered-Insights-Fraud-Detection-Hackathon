...
6. Once the storage account is created,create a container "refined-data" and upload all 4 csv files into the container (inside a folder with name csv-data or txt-data) ,so that at the end of the activity you will have a storage account with a container inside and a subfolder inside a container and that subfolder will have all the files in CSV format ,Once everything is done it will look like below 
...
4. **Storage Account Datastore Configuration**
    a. The best pratise is to use Microsoft Entra Autentication
...
### 5. Create Connections to the Blob Storage & AI Search
    ...
    a. We have multiple ways to achieve this, but the best way is to go to the Management Center in [ai.azure.com] or go to [ml.azure.com] and create connectionsand under connected resources, add a new connection.
...
e. When the deployment has succeeded, select it. Then, on its **Test** page, and review the response.
...
b. Create a new deployment of the `gpt-35-turbo-instruct` model with the following settings by selecting **Customize** afer the **Confirm** button is pressed, in the Deploy model wizard:
    ...
    - **Tokens per Minute Rate Limit (thousands)**: Slide it to maximum
    - **Enable dynamic quota**: 
    Post this click of **Create resource and deploy**
    > **Note**: If your current AI resource location doesn’t have quota available for the model you want to deploy, you will be asked to choose a different location where a new AI resource will be created and connected to your project.\
c. Test the chatbot with a question "What do you do?" and check the response.

Now we will go to the base machine from which we are working so that this OpenAI service can connected to our CSV and enable it so that we can talk to this CSV and ask all queries in the human readable format and will get response 
